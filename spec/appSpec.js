...
describe("resource-sameorigin", function() {
    ...
    it("=> check offline folder with same origin js files without uglify", function(cb) {
        ...
        expect(fs.existsSync('specWebpack/dist/resource-sameorigin-withoutuglify/offline.zip')).toBe(true);
      
        var offline = path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline'),
            localhost = fs.readdirSync(offline);

        expect(localhost[0]).toBe('localhost');

        var localhost = path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline/localhost'),
            ports = fs.readdirSync(localhost);
      
        expect(ports[0]).toBe('8000');
        expect(ports[1]).toBe('9000');

        var port9000 = path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline/localhost/9000'),
            htmlFolder = fs.readdirSync(port9000),
            jsFolder = fs.readdirSync(path.join(port9000, 'js'));
        ...
        var port8000 = path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline/localhost/8000'),
            cssFolder = fs.readdirSync(path.join(port8000, 'css'));

        expect(cssFolder[0]).toBe('index.css');

        var libs = path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline/localhost/9000/js/libs/'),
            libsFolder = fs.readdirSync(libs);

        expect(libsFolder[0]).toBe('react.js');

        let htmlContent = fs.readFileSync(path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline/localhost/9000/entry.html'), "utf-8");
        ...
        let jsContent = fs.readFileSync(path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline/localhost/9000/js/index.js'), "utf-8");
        matchCount = 0;
        ...
        decompress(path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/offline.zip'), path.resolve('specWebpack/dist/resource-sameorigin-withoutuglify/unzip')).then(files => {
            let filesArr = [];
